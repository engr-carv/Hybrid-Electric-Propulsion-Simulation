...
class MissionCharacteristics:
    ...
    temp: np.ndarray # Temperature ratio array
...
def mission_char(cruise_alt, gamma):
    ...
    temps = np.zeros((len(altitude_range), 1), dtype='d')
    ...
    for i, alt in enumerate(altitude_range):
        rho, a, T = isa_properties(alt, gamma)
        speeds_of_sound[i] = a
        densities[i] = rho
        temps[i] = T

    return MissionCharacteristics(a=speeds_of_sound, rho=densities, temp=temps)
