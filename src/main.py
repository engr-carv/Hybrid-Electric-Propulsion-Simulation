...
massFuel = np.array([[6500, 9000, 12750, 18500], 
                        [5750, 8000, 11500, 16500], 
                        [5000, 7250, 10250, 15000], 
                        [4500, 6500, 9500, 15000], 
                        [4250, 6000, 8750, 13750], 
                        [4000, 5500, 8000, 11750]])

massBattery = np.array([[9000, 7000, 3250, 2500], 
                        [10000, 8000, 4500, 375], 
                        [11000, 8500, 5750, 1000], 
                        [11000, 9500, 6500, 2250], 
                        [11000, 10000, 7250, 3500], 
                        [11000, 10500, 8000, 4250]])
...
''' MAIN ANALYSIS LOOP '''
for TOconfig_idx, TOconfig in enumerate(TOconfigs):
    for climbConfig_idx, climbConfig in enumerate(climbConfigs):
        for cruiseConfig_idx, cruiseConfig in enumerate(cruiseConfigs):
            ...
            for rng_idx, mission_range in enumerate(RANGE):
                ...
                carpet_plot.to_excel(f"eta_thermVsbattSPEMTOW_Config {configCounter}.xlsx", f"Rng={mission_ranges[rng_idx]}")
