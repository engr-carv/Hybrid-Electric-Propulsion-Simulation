...
#massAircraft = AC["OperatingEmptyMass"] + massFuel + massPayload + massBattery

#if massAircraft > AC["MTOW"]:
...
roc = np.array([2000, 2000, 2000, 1200]) * FT_TO_MET / 60                       # [fpm] -> [m/s]
...
#a_x = ((cruise_M * miss.a[int(10*phase_altitudes[2])]) - v_x_init) / total_climb_time         # constant acceleration from 10k feet to 24k feet (start of mach climb)
...
for rng_idx, mission_range in enumerate(RANGE):
    ...
    for k, eta_therm in enumerate(eta_thermal):
        ...
        for i, batt_spe in enumerate(specificEnergyArray):
            ...
            batt_m = 12e3
            m_fuel_total = 0
            prev_m_fuel_total = float("inf")
            
            while abs(m_fuel_total - prev_m_fuel_total) > tol:
                ...
                if abs(m_fuel_total - prev_m_fuel_total) > tol:
                    prev_m_fuel_total = m_fuel_total
                    batt_m = batt_m - 500
                    
                '''END OF BATTERY OPTIMIZATION LOOP'''
                
            '''BEGIN FLIGHT SIM AGAIN WITH OPTIMIZED VALUES'''
            
            massFuel = m_fuel_total + 100
            massAircraft = AC["OperatingEmptyMass"] + massPayload + massFuel + batt_m
            weightAircraft = massAircraft * g
            
            fuelMassArray.append(massFuel)
            batteryMassArray.append(batt_m)
            aircraftMassArray.append(massAircraft)
    ...
    carpet_plot.to_excel("eta_thermVsbattSPETEST.xlsx", f"Rng={mission_ranges[rng_idx]}")
    '''
